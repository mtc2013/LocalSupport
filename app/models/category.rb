...
class Category < ActiveRecord::Base
  ...
  def self.html_drop_down_options
    self.where(charity_commission_id_is_less_than(199)).order(table[:name].desc).collect {|category| [ category.name, category.id ] }
  end
  ...
  private
    def self.choose_first_in_range lower, upper
      ...
      category = self.where(charity_commission_id_is_less_than(upper).
        and(charity_commission_id_is_greater_than(lower))).sort!.first
    ...
    def self.charity_commission_id_is_greater_than cat_id
      table[:charity_commission_id].gt(cat_id)
    end

    def self.charity_commission_id_is_less_than cat_id
      table[:charity_commission_id].lt(cat_id)
    end
